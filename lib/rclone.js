...
async function readTextFile(path, cancellable = null) {
    try {
 ...
 */
export async function activeMounts(cancellable = null) {
    const mounted = new Set();
    const contents = await readTextFile(MOUNTS_FILE, cancellable);
    if (contents === null)
        return mounted;
...
class Delay {
    constructor() {
        this._sourceId = 0;
        this._reject = null;
        pendingDelays.add(this);
    }

    wait(ms) {
        return new Promise((resolve, reject) => {
            if (!pendingDelays.has(this)) {
            ...
            this._reject = reject;
            this._sourceId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, ms, () => {
                this._sourceId = 0;
                this._reject = null;
                resolve();
...
/**
 * Drops every poll timer. Call from disable() so sources do not outlive the
 * extension even if a callback would later return SOURCE_REMOVE.
 */
...
export async function waitUntilPopulated(mountpoint, timeoutMs = 30000, cancellable = null) {
    ...
    if (cancellable) {
        ...
        cancelId = cancellable.connect(() => delay.destroy());
    }
